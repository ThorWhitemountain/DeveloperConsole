...
namespace Anarkila.DeveloperConsole
{
    public static class CreateTextFileUtility
    {
        public static bool GenerateCommandList()
        {
            ...
            string path = GetScriptRootPath("CreateTextFileUtility", true);
        ...
        private static void MakeTextFile(string path)
        {
            ...
            List<ConsoleCommandData> commands = CommandDatabase.GetConsoleCommandAttributes(Debug.isDebugBuild, false);
            ...
            for (int i = 0; i < commands.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        string x = string.Format("Parameter {0}: {1}", k + 1, commands[i].parameters[k].FullName);
                        writer.WriteLine(x);
                    }
                }
        ...
        private static string GetScriptRootPath(string scriptname, bool parse = false)
        {
            ...
            string filter = string.Format("t:Script {0}", scriptname);
            string[] asset = AssetDatabase.FindAssets(filter);
